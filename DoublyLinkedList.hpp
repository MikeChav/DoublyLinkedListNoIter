...
class DoublyLinkedList{
    ...
    */
    ...
    /*
    Description:
        - `friend` method to print the contents of the current list using ostreams, such as `std::cout`
        - Each element must be separated by a single space, and the end of the output should contain only the last element of the list (no white space)
        - Example: if list called `list` contains 3, 4, and 5 in order then `std::cout << list;` should print "3 4 5".
    Parameters:
        - std::ostream& os: the stream to add to
        - const DoublyLinkedList<T>& list: the list to output to `os`
    Returns:
        - The modified ostream object
    Exceptions: None
    */
    template <class S>
    friend std::ostream& operator<<(std::ostream& stream, const DoublyLinkedList<S>& list);
};

// Because our class is templated, we need to provide the definitions with the header information
